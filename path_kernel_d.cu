__global__ void get_max_array_double(complex<double>* sol, int dim, int dimLog2, double* max_delta_x ) {
	__shared__ double delta_x[max_array_size];

	int j = threadIdx.x;
	// max for the norm
	delta_x[j] = sol[j].real*sol[j].real + sol[j].imag*sol[j].imag;
...
__global__ void small_backsubstitution_with_delta(complex<double>* R, complex<double>* sol0, int dim, int dimLog2, double* max_delta_x)
{
	int j = threadIdx.x;
	__shared__ complex<double> sol[shmemsize/2];
	__shared__ complex<double> Rcl[shmemsize/2];
	__shared__ double delta_x[shmemsize/2];
	int ind;
	complex<double> update;
