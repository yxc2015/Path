__global__ void get_max_array_double(complex<gqd_real>*sol, int dim, int dimLog2, double* max_delta_x ) {
...
__global__ void small_backsubstitution_with_delta(complex<gqd_real>* R, complex<gqd_real>* sol0, int dim, int dimLog2, double* max_delta_x)
{
	int j = threadIdx.x;
	__shared__ complex<gqd_real> sol[shmemsize/2];
	__shared__ complex<gqd_real> Rcl[shmemsize/2];
	__shared__ double delta_x[shmemsize/2];
	int ind;
	complex<gqd_real> update;
