...
void eval_mon(GPUWorkspace& workspace, const GPUInst& inst, int n_sys){
	...
	if(eval_method == 0){
		...
		/*eval_mon_global_kernel<<<inst.mon_global_grid, inst.mon_global_BS>>>(
				...
				inst.mon_pos_start + inst.n_mon_level[0], inst.mon_pos,
				inst.n_mon_global);*/
		/*std::cout << "inst.NB_mon_block = " <<  inst.NB_mon_block << "inst.BS_mon_block = "<< inst.BS_mon_block<< std::endl;
		std::cout << "inst.n_mon = " <<  inst.n_mon << std::endl;
		std::cout << "inst.n_mon_level[0] = " <<  inst.n_mon_level[0] << std::endl;
		std::cout << "inst.n_mon_block = " <<  inst.n_mon_block << std::endl;*/
