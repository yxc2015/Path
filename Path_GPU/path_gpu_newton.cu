#include "path_gpu_eval.cu"
#include "path_gpu_mgs.cu"
...
__global__ void get_max_array_double(GT* sol, int dim, int dimLog2, double* max_delta_x ) {
...
bool newton(GPUWorkspace& workspace, const GPUInst& inst, Parameter path_parameter) {
	...
	for (int i = 0; i < path_parameter.max_it; i++) {
		...
		if (inst.dim <= BS_QR) {
			...
			get_max_array_double<<<1,inst.dim>>>(workspace.sol, inst.dim, dimLog2, max_delta_x_gpu);
		}
