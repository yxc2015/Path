...
bool GPU_Path(CPUInstHom& hom, Parameter path_parameter, CT* cpu_sol0, CT cpu_t, CT*& x_gpu, int n_path, int inverse) {
	...
	GPUWorkspace workspace(inst.n_workspace, inst.mon_pos_size, inst.n_coef, inst.n_constant, \
...
int path_mult(GPUWorkspace& workspace, GPUInst& inst, Parameter path_parameter, CT* cpu_t, int inverse = 0) {
	...
	while(true) {
		...
		predict_newton_kernel<<<predict_grid, inst.predict_BS>>>( \
				workspace.x_array_mult, \
				workspace.t_array_mult, workspace.n_predictor, \
